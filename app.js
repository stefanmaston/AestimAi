...
function filterLabProducts(cat) {
...
async function labCheckout(productId) {
  const overlay  = document.getElementById('labCheckoutOverlay');
  const content  = document.getElementById('labCheckoutContent');
  overlay.classList.remove('hidden');
  content.innerHTML = '<div class="lab-checkout-spinner">Ansluter till kassan…</div>';
  ...
  try {
    ...
    if (data.url) {
      ...
      content.innerHTML = `<div class="lab-checkout-error">
        <p>${data.error || 'Något gick fel. Försök igen.'}</p>
        <button class="btn-secondary" onclick="closeLabCheckout()">Stäng</button>
      </div>`;
    }
  } catch (err) {
    content.innerHTML = `<div class="lab-checkout-error">
      <p>Kunde inte ansluta till betalningssystemet. Kontrollera din internetanslutning.</p>
      <button class="btn-secondary" onclick="closeLabCheckout()">Stäng</button>
    </div>`;
  }
}

function closeLabCheckout() {
  document.getElementById('labCheckoutOverlay').classList.add('hidden');
}

window.filterLabProducts = filterLabProducts;
window.labCheckout       = labCheckout;
window.closeLabCheckout  = closeLabCheckout;
